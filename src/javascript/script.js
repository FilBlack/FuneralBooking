...
// Dynamic Funeral Form
document.addEventListener('DOMContentLoaded', function() {
	var funeral_type = document.getElementById("funeral_type");
  
	funeral_type.addEventListener('change', function() {
	  var selectedType = this.value;
  
	  if (selectedType === 'cremation') {
		// Handle Burial elements
		document.querySelectorAll('.burial-visible').forEach(el => {
		  el.classList.add('burial');
		  el.classList.remove('burial-visible');
		  el.style.display = 'none'; // Match this duration with the animation duration
		});
  
		// Handle Cremation elements
		document.querySelectorAll('.cremation').forEach(el => {
		  el.style.display = 'block';
		  setTimeout(() => {
			el.classList.add('cremation-visible');
			el.classList.remove('cremation');
		  }, 10); // Delay to ensure display:block takes effect
		});
	  } else if (selectedType === 'burial') {
		// Handle Cremation elements
		document.querySelectorAll('.cremation-visible').forEach(el => {
		  el.classList.remove('cremation-visible');
		  el.classList.add('cremation');
		  el.style.display = 'none';
		});
  
		// Handle Burial elements
		document.querySelectorAll('.burial').forEach(el => {
		  el.style.display = 'block';
		  setTimeout(() => {
			el.classList.add('burial-visible');
			el.classList.remove('burial');
		  }, 10); // Delay to ensure display:block takes effect
		});
	  }
	});
	...
	var photo_selection_yes = document.getElementById("photo_yes");
	var photo_selection_no = document.getElementById("photo_no");
	var photo_upload = document.getElementById("photo_upload");
	
	function togglePhotoUpload() {
		if (photo_selection_yes.checked) {
			setTimeout(() => {
				photo_upload.classList.add('visible');
				photo_upload.classList.remove('hidden');
			}, 10);
		} else if (photo_selection_no.checked) {
		photo_upload.classList.add('hidden');
		photo_upload.classList.remove('visible');
		}
	}
	...
	photo_selection_yes.addEventListener('change', togglePhotoUpload);
	photo_selection_no.addEventListener('change', togglePhotoUpload);

	var music_selection_yes = document.getElementById("music_yes");
	var music_selection_no = document.getElementById("music_no");
	var music_text = document.getElementById("music_text");

	function toggleMusic() {
		if (music_selection_yes.checked) {
			music_text.style.display = 'block';
			setTimeout(() => {
				music_text.classList.add('visible');
				music_text.classList.remove('hidden');
			}, 10); // Small delay to ensure display:block takes effect
		} else if (music_selection_no.checked) {
			music_text.classList.add('hidden');
			music_text.classList.remove('visible');
			setTimeout(() => {
	...
	// Add event listeners to both radio buttons
	music_selection_yes.addEventListener('change', toggleMusic);
	music_selection_no.addEventListener('change', toggleMusic);

	// Address and email address autosave
		//autocomplete is handled when the user selects the address 
	...
	var package1 = document.getElementById("package1_button")
		package1.addEventListener('click', () => {
		// Set service type to Cremation
		document.getElementById('funeral_type').value = 'cremation';

		// Set service philosophy to Humanist
		document.getElementById('humanist').checked = true; // Assuming 'yes' is for Humanist
	
		// Set service location to Crematorium
		document.getElementById('church').checked = true; // Assuming this is the correct input for Crematorium
	
		// Set Coffin to Basic Coffin
		document.getElementById('coffin').value = 'Basic coffin';
		
		document.getElementById('removal_day_of').checked = true;

		document.getElementById('embalm_yes').checked = true;

		document.getElementById('singer_no').checked = true;
		
		// Deselect all services not included
		document.getElementById('limousine_no').checked = true;
		document.getElementById('notice_mail').checked = true; // Assuming 'mail later' implies not included
		document.getElementById('music_no').checked = true;
		document.getElementById('booklets_no').checked = true;
		document.getElementById('photo_no').checked = true;
		document.getElementById('grave_marker_no').checked = true;
		document.getElementById('carry_no').checked = true; // Assuming family does not carry coffin
		document.getElementById('wake_funeral_home').checked = true; // Assuming 'wake at funeral home' is not include
	});

	document.getElementById("package2_button").addEventListener('click', () => {
		document.getElementById('funeral_type').value = 'burial';
		document.getElementById('catholic').checked = true; // Assuming 'yes' is for Catholic
		document.getElementById('church').checked = true;
		document.getElementById('coffin').value = 'Standard coffin';
		document.getElementById('removal_day_before').checked = true;
		document.getElementById('embalm_yes').checked = true;
		document.getElementById('limousine_yes').checked = true;
		document.getElementById('notice_write').checked = true;
		document.getElementById('music_yes').checked = true;
		document.getElementById('booklets_yes').checked = true;
		document.getElementById('photo_yes').checked = true;
		document.getElementById('grave_marker_yes').checked = true;
		document.getElementById('carry_yes').checked = true;
		document.getElementById('wake_home').checked = true;
	});
	
	document.getElementById("package_eco_button").addEventListener('click', () => {
		console.log("eco package triggered")
		document.getElementById('funeral_type').value = 'cremation';
		document.getElementById('humanist').checked = true; // Assuming 'no' is for Humanist
		document.getElementById('crematorium').checked = true; // Make sure to add an ID for this input
		document.getElementById('coffin').value = 'Eco-Friendly Coffin';
		document.getElementById('embalm_no').checked = true;
		document.getElementById('limousine_no').checked = true;
		document.getElementById('notice_mail').checked = true;
		document.getElementById('music_yes').checked = true;
		document.getElementById('booklets_no').checked = true;
		document.getElementById('photo_no').checked = true;
		document.getElementById('grave_marker_no').checked = true;
		document.getElementById('carry_no').checked = true;
		document.getElementById('wake_funeral_home').checked = true;
	});
	
	document.getElementById("package3_button").addEventListener('click', () => {
		document.getElementById('funeral_type').value = 'burial';
		document.getElementById('catholic').checked = true; // Assuming 'yes' is for Catholic
		document.getElementById('church').checked = true;
		document.getElementById('coffin').value = 'Premium Coffin';
		document.getElementById('removal_day_before').checked = true;
		document.getElementById('embalm_yes').checked = true;
		document.getElementById('limousine_yes').checked = true;
		document.getElementById('notice_write').checked = true;
